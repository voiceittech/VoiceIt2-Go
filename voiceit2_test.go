...
func TestVideo(t *testing.T) {
	...
	var daefu structs.DeleteAllEnrollmentsForUserReturn
	json.Unmarshal([]byte(ret), &daefu)
	assert.Equal(200, daefu.Status, "message: "+daefu.Message)
	assert.Equal("SUCC", daefu.ResponseCode, "message: "+daefu.Message)

	ret = myVoiceIt.GetAllVideoEnrollments(userId2)
	var gve4 structs.GetAllVideoEnrollmentsReturn
	json.Unmarshal([]byte(ret), &gve4)
	assert.Equal(200, gve4.Status, "message: "+daefu.Message)
	assert.Equal("SUCC", gve4.ResponseCode, "message: "+daefu.Message)
	assert.Equal(0, len(gve4.VideoEnrollments), "message: "+daefu.Message)
